...
class AuthController extends Controller
{
    ...
    public function login(Request $req){
        ...
        if (!$user||!Hash::check($fiels['password'], $user->password)) {
            return response(['user'=>$user,'message'=>'email or password invalid','isValidEmail'=>true], 422);
        }

        $token=$user->createToken('secrettoken')->plainTextToken;
        return response(['user'=>$user,'message'=>'loggedin','token'=>$token,'isValidEmail'=>true], 200);
    }
}
