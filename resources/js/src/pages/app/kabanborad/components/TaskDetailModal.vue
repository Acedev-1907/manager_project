...
import { subscribeTaskComments, unsubscribeTaskComments } from '../../../../helper/taskCommentsRealtime';
import eventBus from '../../../../helper/eventBus';
import { getCurrentUserId, getCurrentUserData } from '../../../../helper/getUserData';
...
const currentUserId = computed(() => {
    ...
    // Fallback: dùng helper chuẩn hoá
...
const currentUser = computed(() => {
    ...
    // Fallback: lấy từ local storage helper
    return getCurrentUserData()?.user || null;
});
...
// Khi mở modal (visible true) thì load lại comments, setup realtime và lock scroll body
watch(
    () => props.visible,
    (visible) => {
        const tid = getTaskId();
        if (visible && tid) {
            ...
            newComment.value = '';
            loadComments();
            subscribeTaskComments(Number(tid));
            lockBodyScroll();
        } else {
            if (tid) {
                unsubscribeTaskComments(Number(tid));
            }
            unlockBodyScroll();
        }
    }
...
// Lắng nghe event comment realtime qua eventBus (tập trung 1 nơi)
function onTaskCommentCreated(payload: unknown) {
    const data = payload as { taskId: number; comment: any };
    const tid = getTaskId();
    if (!tid || Number(tid) !== Number(data.taskId)) return;
    handleCommentEvent(data.comment);
}

onMounted(() => {
    eventBus.on('task-comment-created', onTaskCommentCreated);
});
...
onUnmounted(() => {
    ...
    eventBus.off('task-comment-created', onTaskCommentCreated);
    unlockBodyScroll();
});
