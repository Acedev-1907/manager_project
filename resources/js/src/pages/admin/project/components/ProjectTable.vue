            ...
            <tbody>
                <tr v-for="project in projects?.data?.data" :key="project.id">
                    ...
                    <td>
                        ...
                        <div class="progress" role="progressbar" aria-label="Example with label" aria-valuenow="5"
                            aria-valuemin="0" aria-valuemax="100">
                            <div class="progress-bar bg-success"
                                :style="{ width: project?.task_progress?.progress + '%' }">
                                {{ project?.task_progress?.progress }} %</div>
                        </div>
                    ...
                    <td>
                        <button @click="emit('pinnedProject', project.id)" type="button" class="btn btn-light">Pinned <i
                                class="bi bi-activity"></i></button>
                    </td>

                    <td>
                        <RouterLink class="btn btn-warning" :to="'/kaban?query=' + project.slug">View <i
